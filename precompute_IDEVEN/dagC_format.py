    ...
    while(i<=len(spe_list)-1):
        ...
        while(j<=len(spe_list)-1):
            if spe_list[i]==specie or spe_list[j]==specie :
                ...
                cmd="module load system/python/2.7.9; qsub -V -q normal.q -N  dag_tools -b y  '"+pathCoge+"web/bin/SynMap/dag_tools.py -q a"+code_spe_1+" -s b"+code_spe_2+" -b ./"+code_spe_1+"-"+code_spe_2+".CDS-CDS.last.tdd10.cs0.filtered -c > ./"+code_spe_1+"-"+code_spe_2+".CDS-CDS.last.tdd10.cs0.filtered.dag.all'"
